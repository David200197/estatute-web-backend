...
export class AboutUs extends Entity<AboutUsProps> implements AboutUsModel {
  ...
  public static create(options: SelfPartial<AboutUsProps, 'uuid'>): AboutUs {
    ...
    const scientificCategory = ScientificCategory.create(
      options.scientificCategory,
    );
    const specialty = Specialty.create(options.specialty);
    const yearsOfWorkExperience = YearsOfWorkExperience.create(
      options.yearsOfWorkExperience,
    );
    const teachingExperience = TeachingExperience.create(
      options.teachingExperience,
    );
    const teachingCategory = TeachingCategory.create(options.teachingCategory);
    const investigativeCategory = InvestigativeCategory.create(
