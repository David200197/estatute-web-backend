...
export interface EntitiesModel<
  Model extends EntityModel<Record<string, unknown>>,
> extends ValueObject<Model[]> {
   ...
   */
  groupBy(
    getKey: (event: Model) => number | string,
  ): Record<number | string, Model[]>;
  /**
   ...
   */
  select(
    options: Partial<OptionsFlags<Model['value']>>,
  ...
  implements EntitiesModel<Model>
{
  ...
  clone(): Entities<Model> {
    return new Entities([
      ...this.value.map((data) => data.clone()),
    ]) as Entities<Model>;
  }
